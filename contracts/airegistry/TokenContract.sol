...
/// nothing is claimable until `acceptProbe` takes it. Past that the escrow stays
...
/// `_bondAmount()`,) before `open()`. There is no prepaid/frozen buffer beyond the
...
/// DISPUTE reaches ONLY the contested delta `claimed - trusted`; everything trusted
...
/// 5. `withdrawShell`/`destroy` -- seller pulls finalized SHELL.
contract TokenContract is AiRegistryModifiers {
    ...
    // code -- it reads like something the contract still relies on.
    ...
    uint256 _buyerPubkey; // buyer note pubkey
    ...
    // through the match for the gateway.
    ...
    uint128 _sellerBond; // SHELL held as the seller's mirror collateral (up to 2P),
    ...
    uint128 _feeAccrued; // SHELL fee charged by-fact on finalized ticks
    uint128 _ticksFinalized; // count of finalized ticks
    bool    _everDisputed; // a dispute ever opened -> no rebate
    uint64  _fundedTime; // when funded
    ...
    constructor(
        ...
        // maxTicks*(price + fee) must fit uint128 so every downstream unitxticks total (the
        ...
        // AN ADDRESS FIELD THAT IS ONLY EVER READ MUST STILL BE WRITTEN ONCE. A declared and
    ...
    /// @notice Burn this entry's measured SHELL charge.
    ...
    /// Sized so the reserve covers a whole deal: ~0.215 constant + 0.013 per tick.
    ...
    /// @notice Burn SHELL -- generation 4.0.33.
    ...
    /// @notice Platform fee (2.5%, PLATFORM_FEE_BPS) of `amount`.
    ...
    /// @notice Seller mirror bond: two ticks, for every deal shape. A dispute is about
    ...
    /// platform fee is separate and not part of this bond.
    function _bondAmount() private view returns (uint128) {
        return 2 * _pricePerTick;
    }

    /// @notice Seller rebate for `n` cleanly-finalized ticks at price P:
    ...
    /// clean, never-disputed close,) and burn the net.
    ...
    function _recordFunding(address buyer, uint256 buyerPubkey, uint128 paid) private {
        // Buyer-side, by-fact fee: the escrow covers per-tick (P + fee).
    ...
    /// @notice Order-book handover: the InferenceOrderBook forwards
    ...
    /// @notice Note-driven, single-call sell-offer post. The seller's
    ...
    // ========================================================
    // 1b. Seller bond -- seller posts the mirror collateral 2P
    // ========================================================
    ...
    /// What arrives is the mirror bond: held until close, returned to the note
    ...
    /// freezes ONE tick as the probe. The buyer's
    /// note is NOT locked -- the seller's risk is the per-deal mirror bond, see the
    ...
    function open(bytes endpointCipher) public onlyOwnerPubkey(_sellerPubkey) accept {
        ...
        // No note lock: the buyer's at-risk SHELL is the deposit held in THIS TC,
    ...
    /// a live endpoint is consent. The tick is credited, its fee taken by-fact,
    ...
    /// simplification./: outside a dispute the seller is credited the full
    ...
    /// the buyer's `stop()` -- where the started week is charged in full -- and would
    /// hand the seller a started week he is not entitled to on `sellerStop`, where says
    ...
    /// and the seller rebate both measure WORK DONE, so a take-or-pay week the
    ...
    /// @notice Terminal, no dispute ever opened: pay the rebate, burn the net fee,
    ...
    function stop() public {
        ...
        // Before this, walking out inside the window cost the buyer nothing.
        if (!_isSubscription()) {
            if (_tokensPend > _tokensFinal) { _tokensFinal = _tokensPend; }
    ...
    // ========================================================
    // 4b. Dispute -- the buyer's standing right to challenge the deal
    // ========================================================
    ...
    function resolveDisputeTimeout() public {
        ...
        ensureBalance();
        _chargeGas(GAS_TERMINAL);
        // Nobody agreed, so nobody is believed: an EQUAL `D` is destroyed on each side. The
    ...
    function _settleTrustedAndClose(uint128 buyerStake, uint128 sellerStake) private {
        ...
        _finalizedOwed += _sellerBond; _sellerBond = 0;
        _opened = false;
        _settleFees(false); // a dispute ever opened -> no rebate
        if (refund > 0) { _payShell(_buyer, refund); }
    ...
    /// @notice Seller bond state: whether the seller posted the mirror
    ...
    /// @notice Buyer bond: the `2P` posted with the escrow at funding and
    ...
    /// @notice Fee state: accrued fee, finalized-tick count (rebate n),
    ...
    /// @notice Buyer note pubkey recorded at the match: the gateway
