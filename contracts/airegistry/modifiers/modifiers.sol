...
abstract contract AiRegistryModifiers is AiRegistryErrors {
    ...
    // Platform fee: 2.5%, buyer-side, BY-FACT (on
    // delivered ticks), net-of-rebate burned. NOT charged upfront.
    uint16 constant PLATFORM_FEE_BPS = 250; // 2.5%
    // Seller rebate: rate = min(REBATE_MAX_BPS, REBATE_SLOPE_BPS * n) bps, paid only on a
    ...
    // Probe phase: the buyer's window to try the service before anything can be
    ...
    uint64  constant MATCH_OPEN_TIMEOUT = 600; // funded-but-unopened cleanup
    ...
    // The figures come from the measured campaign in, rounded UP to the next 0.005: the
    ...
    // margin. Sizing a whole deal is a separate question, answered by the formula in
    ...
    // Entries the run never exercised, priced by the closest measured neighbour rather than
    ...
    // InferenceOrderBook -- dedicated 1000+ range (separate from registry/streaming/oracle 700s)
    ...
    // InferenceOrderBook -- continue the 1000+ range
