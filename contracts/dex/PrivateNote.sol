...
/// note must be the bound buyer note.
...
contract PrivateNote is Modifiers, ReplayProtection {
    ...
    /// @notice Max lifetime (seconds) of a resting SELL offer. A SELL commits NO
    ...
    /// @notice InferenceOrderBook code baked at deploy by RootPN. The note
    ...
    /// @notice TokenContract code baked at deploy by RootPN, for the SAME reason the book code
    ...
    /// @param inferenceOrderBookCode InferenceOrderBook code for deterministic book derivation.
    ...
    // -- Inference market: deploy an InferenceOrderBook FROM this note --
    ...
    /// so a given model maps to exactly one book.
    ...
    // -- Inference market: note as active participant ------------
    // Inference settles in SHELL -- held physically by the note
    ...
    /// @param ttl Mandatory offer lifetime in seconds, `1 <= ttl <= MAX_SELL_TTL`.
    ...
    /// @notice Send a BUY order from this note with SHELL escrow.
    ...
    function placeInferenceBuy(
        ...
        // forward this note's pubkey so the OB can record it in the deal
    ...
    // The legacy subscription entry point (resting weekly-cycle bid) was removed with the
    ...
    /// @notice Buyer note stops the stream -- amicable exit. Max loss
    ...
    /// @notice Buyer note disputes the current <=2 ticks. Neither note is locked by
    /// this -- no note is locked by a deal at all. Every stake a dispute can reach
    ...
    /// burns its measured charge from -- running it down costs the seller a
    ...
    /// ) sized a mechanism that no longer exists, when every entry drew on the reserve.
