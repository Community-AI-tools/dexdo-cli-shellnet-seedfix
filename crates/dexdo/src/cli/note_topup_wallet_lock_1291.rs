...
fn topup_args(
    ...
    crate::cli::args::NoteTopupArgs {
        ...
        contracts: manifest.to_path_buf(),
...
async fn note_topup_refuses_while_note_deploy_holds_the_funding_wallet_1291() {
    ...
    let manifest = write_manifest(temp.path(), "shellnet");
    ...
    let outcome = super::run_note_topup(topup_args(&manifest, &legacy_wallet)).await;
    match previous {
    ...
    drop(held);
    let path = super::funding_wallet_lock_path("shellnet", &legacy_wallet).expect("lock path");
...
fn funding_wallet_turn_is_released_when_the_holder_drops_1291() {
    ...
    drop(regained);
    let path = super::funding_wallet_lock_path("shellnet", &wallet).expect("lock path");
    let _ = std::fs::remove_file(&path);
}
...
fn funding_wallet_lock_key_separates_networks_and_joins_address_forms_1291() {
    ...
    let shellnet_legacy = super::funding_wallet_lock_path("shellnet", &legacy).expect("legacy");
    let shellnet_canonical =
        super::funding_wallet_lock_path("shellnet", &canonical).expect("canonical");
    let mainnet_legacy = super::funding_wallet_lock_path("mainnet", &legacy).expect("mainnet");

    assert_eq!(
        shellnet_legacy, shellnet_canonical,
        "one wallet written two ways must take ONE turn, or the two commands never see each other"
    );
    assert_ne!(
        shellnet_legacy, mainnet_legacy,
    ...
    assert!(
        super::funding_wallet_lock_path("shellnet", "not-an-address").is_err(),
